...
const StudentAssignments = () => {
    ...
    const isOverdue = (assignment) => {
        if (assignment.isSubmitted) return false; // Not overdue if submitted
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {assignments.map((assignment) => (
                        <Link
                            ...
                            key={assignment._id}
                            className="card group hover:no-underline block h-full flex flex-col"
                        >
                            <div className="flex justify-between items-start mb-4">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${assignment.type === 'AI_Generated'
                            ...
                            <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2 line-clamp-2 group-hover:text-primary transition-colors">
                                {assignment.title}
                            </h3>

                            <div className="text-sm text-gray-600 dark:text-gray-400 mb-6 flex items-center gap-2">
                                <span className="font-semibold px-2 py-0.5 bg-gray-100 dark:bg-white/10 rounded text-gray-700 dark:text-gray-300 text-xs">{assignment.courseId?.courseCode}</span>
                                <span>{assignment.courseId?.title}</span>
                            </div>

                            <div className="mt-auto flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 pt-4 border-t border-gray-100 dark:border-white/5">
